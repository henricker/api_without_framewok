...
export class NativeHttpControllerAdapter {
    ...
    static extractFileFromBuffer(buffer: Buffer): FileRequest | null {
        ...
        const contentDispositionRegex = /Content-Disposition: form-data; name="file"; filename="([^"]+)"/;
    ...
    static responseErrorHandling(error: Error & { statusCode: number, details: any }, response: ServerResponse) {
        response.statusCode = error.statusCode || 500
        let message = response.statusCode !== 500 ? error.message : 'Internal Server Error'
        return response.end(JSON.stringify({ message, details: response.statusCode === 422 ? error.details : undefined }))
    }
