...
describe('UNIT - #HttpController - UploadCSVFileUsers', () => {
    ...
    const invalidParamsWithoutInvalidMimeType = {
        file: {
            buffer: Buffer.from('name,city,country,favorite_sport\nJohn Doe,New York,USA,Basketball\nJane Smith,London,UK,Football', 'utf-8'),
            mimeType: 'text/txt',
            filename: 'file.txt'
        }
    } as IHttpRequest
   ...
   it('Should throw ValidationError if file not found', async () => {
    const { sut } = makeSut()
    //Act & Assert
    await assert.rejects(sut.execute(invalidParamsWithoutFile), new ValidationError([{ field: 'file', error: 'file is required' }]))
   })

   it('Should throw ValidationError if mimeType of file is not text/csv', async () => {
    const { sut } = makeSut()
    //Act & Assert
    await assert.rejects(sut.execute(invalidParamsWithoutInvalidMimeType), new ValidationError([{ field: 'file', error: 'Invalid mimetype file' }]))
   })
   ...
   it('should call createUserUseCase twice after parsing CSV with two users', async (t) => {
    ...
    //Assert
    const calls = (createUserUseCase.execute as unknown as MockedObject).mock.calls
